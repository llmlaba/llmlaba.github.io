---
layout: default
title: "Open (Fake) Llama 7b CUDA PyTorch Test"
date: 2026-01-12
...
## Dry-run Open Llama 7b

- Preapre python environment for CUDA:

```bash
mkdir -p ~/llm && cd ~/llm
python3 -m venv .venv_llm_openllama
source ./.venv_llm_openllama/bin/activate
python -m pip install --upgrade pip
pip install "torch==2.7.1" "torchvision==0.22.1" "torchaudio==2.7.1" --index-url https://download.pytorch.org/whl/cu128
pip install transformers accelerate
pip install "protobuf<=6.33.3" "tiktoken<=0.12.0" "sentencepiece<=0.2.1"
```
- Get the Open Llama 7b model:

```bash
git lfs install
git clone https://huggingface.co/openlm-research/open_llama_7b openllama
```

- Create script test_cuda_openllama.py:
...
model_path = "/home/sysadmin/llm/openllama"
